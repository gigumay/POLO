...
It must further be noted that POLO is currently missing two functionalities that are present in YOLO models:
1. Data augmentations: The built-in augmentation functionalities of YOLO have not yet been implemented for POLO. This means that none of [these parameters](https://docs.ultralytics.com/modes/train/#augmentation-settings-and-hyperparameters) can be used during training. We will post an update here as soon as this changes. 
2. W&B support: When training YOLO models, users will be prompted to log into weights&biases and a project will be created to monitor training and validation. This feature is missing for POLO as well, but may be implemented in the future.
